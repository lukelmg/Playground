    ...
    var Digit = /** @class */ (function (_super) {
        ...
        Digit.prototype.mathspeak = function (opts) {
            if (opts && opts.createdLeftOf) {
                ...
                var cursorL = cursor[L];
                var cursorLL = cursorL && cursorL[L];
                var cursorParentParentSub = cursor.parent instanceof SupSub
                    ? cursor.parent.parent.sub
                    : undefined;
                if (cursor.options.autoSubscriptNumerals &&
                    cursor.parent !== cursorParentParentSub &&
        ...
        Letter.prototype.autoParenthesize = function (cursor) {
            ...
            while (str.length) {
                if (autoParenthesizedFunctions.hasOwnProperty(str) &&
                    ...
                    // ORBIT EDITED
                    // Check if there's an equals sign between cursor and the start of the line
                    console.log("hello there");
                    var node = cursor[L];
                    var foundEquals = false;
                    while (node) {
                        if (node.ctrlSeq === '=' || node.ctrlSeq === '\\equals') {
                            foundEquals = true;
                            break;
                        }
                        node = node[L];
                    ...
                    if (foundEquals) {
                        // If equals found, move cursor to right of equals before adding parenthesis
                        while (cursor[L] && (cursor[L].ctrlSeq !== '=' && cursor[L].ctrlSeq !== '\\equals')) {
                            cursor.moveLeft();
                        }
                        cursor.moveRight();
                    }
